...
public class ServerKickListener {
    ...
    public ServerKickListener(FallbackServerVelocity plugin) {
        this.plugin = plugin;
    ...
    public void onPlayerKick(KickedFromServerEvent event) {
        ...
        ErrorHandler.add(Severity.ERROR, "[KICK] Server " + kickedName + " kicked player " + player.getUsername() + " for reason: " + kickReasonString);
    ...
    private void handleFallback(KickedFromServerEvent event, RegisteredServer kickedFrom, Player player, String kickReasonString, String kickedName, boolean reconnect) {
        ...
        if (lobbies.isEmpty()) {
            ErrorHandler.add(Severity.ERROR, "[FALLBACK] Null lobbies for player " + player.getUsername() + " in group " + group);
